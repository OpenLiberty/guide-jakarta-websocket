...
https://jakarta.ee/specifications/websocket[Jakarta WebSocket^] enables dual communication between client and server endpoints. First, each client makes a connection to a Jakarta WebSocket server by using HTTP. The server can then broadcast messages to the clients. WebSocket differs from Server-Sent Events (SSE) in that SSE is unidirectional from server to client, whereas WebSocket is bidirectional. WebSocket also enables real-time updates over a smaller bandwidth than SSE.
