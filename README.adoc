...
The [hotspot=onMessageMethod file=0]`onMessage()` method is called when receiving a message through the `option` parameter. According to the [hotspot=loadAverage hotspot=memoryUsageOrBoth file=0]`option` value, construct a message with time, system load and memory usage data, and boardcast the [hotspot=sendToAllSessions file=0]`JsonObject` message. The [hotspot=sendToAllSessionseMethod file=0]`sendToAllSessions()` method uses the Websocket API to boardcast the message to all client sessions.
