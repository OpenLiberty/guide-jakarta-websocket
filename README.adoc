...
https://jakarta.ee/specifications/websocket[Jakarta WebSocket^] enables dual communication between client and server endpoints. First, each client makes a connection over to the server by using HTTP. The server can then broadcast messages to the clients. WebSocket differs from Server-Sent Events (SSE) that SSE is unidirectional from server to client whereas WebSocket is bidirectional. WebSocket also enables the functionality of real-time updates over a smaller bandwidth.
...
You'll learn how to use Jakarta WebSocket API to build the `system` service and the scheduler in the `client` service. The scheduler pushes messages to the `system` service in every 10 seconds. Then, the `system` service will broadcast messages to clients. You will also learn how to use JavaScript `WebSocket` object in HTML file to build a WebSocket connection, subscribe to different events, and display the broadcasting messages from the `system` service in a table.
...
Annotate the `SystemService` class with [hotspot=serverEndpoint file=0]`@ServerEndpoint` annotation to make it as a WebSocket server. The `@ServerEndpoint` 's `value` attribute specifies the URI where the endpoint will be deployed, the `encoders` attribute for the classes to encode messages, and the `decoders` attribute for classes to decode messages. Provide methods that define the WebSocket lifecycle like establishing a connection, receiving a message, and closing the connection by annotating them with the [hotspot=onOpen file=0]`@OnOpen`, [hotspot=onMessage file=0]`@OnMessage` and [hotspot=onClose file=0]`@OnClose` annotations respectively. The method annotated with the [hotspot=onError file=0]`@OnError` annotation is responsible for tackling errors.
...
The required features for the `client` service have been enabled for you in the [hotspot=features file=3]`server.xml` configuration file.
...
Because you are running the `system` and `client` services in dev mode, the changes that you made were automatically picked up. You're now ready to check out your application in your browser.

Point your browser to the http://localhost:9080 URL to test out the `client` service. See that the table is being updated for every 10 seconds.
