...
https://jakarta.ee/specifications/websocket[Jakarta WebSocket^] enables dual communication between client and server endpoints. First, the clients make connection over to the server by using HTTP. The server can broadcast messages to the clients. WebSocket differs from Server-Sent Events (SSE) that SSE is unidirectional from server to client whereas WebSocket is bidirectional. WebSocket also enables the functionality of real-time updates over a smaller bandwidth.
...
You'll learn how to use Jakarta WebSocket API to build the `system` service and the scheduler in the `frontend` service. The scheduler pushes messages to the `system` service. Then, the `system` service will broadcast messages to clients. You will also learn how to use JavaScript `WebSocket` object in HTML file to build WebSocket connection, subscribe on different events, and display the broadcasting messages from the `system` service in a table.
...
Annotate the `SystemService` class with [hotspot=serverEndpoint file=0]`@ServerEndpoint` annotation to make it as a WebSocket server. The `@ServerEndpoint` annotation specifies the `value` attribute for the connection endpoint, the `encoders` attribute for the classes to encode messages, and the `decoders` attribute for classes to decode messages. Provide methods that define the WebSocket lifecycle like establishing a connection, receiving a message, and closing the connection by annotating them with the [hotspot=onOpen file=0]`@OnOpen`, [hotspot=onMessage file=0]`@OnMessage` and [hotspot=onClose file=0]`@OnClose` annotations respectively. The method annotated with the [hotspot=onError file=0]`@OnError` annotation is responsible for tackling errors.

The [hotspot=onOpenMethod file=0]`onOpen()` method stores up the client sessions. The [hotspot=onCloseMethod file=0]`onClose()` method displays the reason of closing connection and removes the closing session from the client sessions.

The [hotspot=onMessageMethod file=0]`onMessage()` method is called when receiving a message through the `option` parameter. According to the [hotspot=loadAverage hotspot=memoryUsageOrBoth file=0]`option` value, construct a message with system load and memory usage data, and send out the [hotspot=sendToAllSessions file=0]`JsonObject` message. The [hotspot=sendToAllSessionseMethod file=0]`sendToAllSessions()` method uses the WebSocket API to broadcast the message to all client sessions.
...
In this section, you will create the WebSocket client that communicates with the WebSocket server, the scheduler that uses the WebSocket client to send messages to the server, and an HTML file that uses JavaScrpt `WebSocket` object to build WebSocket connection, subscribe on different events, and display the broadcasting messages from the `system` service in a table.

On another command-line session, navigate to the `start` directory and run the following goal to start the `frontend` service in dev mode:
...
The `SystemClient` class is used to communicate and test the `system` service. Its implementation is similar to the client class from the `frontend` service that you created in the previous section. At the [hotspot=onMessage file=0]`onMessage()` method, decode and verify the message. 
